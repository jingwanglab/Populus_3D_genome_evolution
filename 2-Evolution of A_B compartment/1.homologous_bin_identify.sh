...
awk  -F'\t' 'NR==FNR{a[$4]=$1"\t"$2"\t"$3;next}NR>FNR{if($4 in a) print $1"\t"$2"\t"$3"\t"a[$4]}' $query.40000.bed map.40000.minm0.7_loc | awk '{if($1==$4) print $0}' > $ref.$query.40kb.minm0.7_homologous.bin

# 2. Conversion status of A/B compartments for homologous bins
for i in Ppse Pwua Psze Plas Pyun Prot Pqio Pdav Pade Psim;
do
   cd /w/00/g/g00/hic/compartment/ab.change.homologous.bin/Pkor."$i"/;
